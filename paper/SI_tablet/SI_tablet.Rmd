...
df %<>%
  ...
  dplyr::filter(selection_type != "someall" & 
      ...
      selection_type != "somesome") %>% #this is filtering out the strange repeats
  dplyr::filter(english >= 0.75) %>% # excluding less than or equal to 75% english exposure
  dplyr::filter(english != "NA")%>% #avoiding the missing value that snuck back in
  ...
  dplyr::filter(agesplit != "NA") #just in case any NAs popped up
...
sub.pars <- select(sub.pars, -Age) %>%
  ...
  left_join(df %>% 
      group_by(sub_id) %>% 
      distinct(age_round, agesplit, accuracy) %>% 
