...
knitr::opts_chunk$set(fig.width=3, fig.height=3, fig.crop = F, fig.pos = "tb", fig.path='figs/',
                      echo=F, warning=F, cache=FALSE, message=F, sanitize = T)
```
...
One possibile cause of children's failures is that they might not have access to the relevant lexical alternatives. This idea---known as the *Alternatives Hypothesis* [@barner2010]---predicts that if children cannot quickly and reliabily bring to mind the quantifier "all" in a situation where they hear "some," they will be unable to make the implicature computation (which on almost all accounts requires negating "all"). The alternatives hypothesis makes a number of predictions about children's abilities in reasoning about quantifier, some of which have been confirmed empirically [e.g., @barner2011].

But it is still unknown whether the full pattern of successes and failures that children show in pragmatic tasks is due to difficulty reasoning about alternatives. Previous studies have used a wide variety of methods, making it difficult to reason about developmental changes in implicature performance. Across tasks, performance appears to vary as a function of the experimental paradigm, syntactic construction for the implicature prompts, age of the child, and dependent variable being meaured [@guasti2005; @horowitz2015; @noveck2001; @papafragou2003; @papafragou2004]. 

In an attempt to make a direct test the Alternatives Hypothesis, Horowitz and Frank [-@horowitz2015] designed a simple referent selection paradigm that could be used across a broad age range (3--5 years) to explore both scalar and ad-hoc (context dependent) implicatures. In this task, children saw three book covers, each featuring four familiar objects (Figure \ref{fig:image}). On target trials, the experimenter described a book using a semantically ambiguous description (e.g., "On the cover of my book, some of the pictures are cats" [scalar] or "On the cover of my book are cats" [ad hoc]). Children succeded on ad-hoc trials but largely failed to make SIs. Interestingly, those same children who failed on SI also failed on "none" trials---in several samples, performance was highly correlated. 

These findings provide some support for the Alternatives Hypothesis---in that SIs were more difficult than ad-hoc trials in a matched paradigm---but also presented a puzzle. Was "none" performance correlated with SI performance because "none" is actually relevant to SIs using "some"? While this idea is not consistent with conventional theories of scalar implicature, some recent investigations suggest the importance of negative alternatives in SIs [@franke2014]. Or alternatively, did this correlation signal a possible alternative developmental factor involved in SI failure, for example the ability to inhibit a more salient alternative ("all" for "some" trials, and positive matches on "none" trials)? The current study addresses this question. 
